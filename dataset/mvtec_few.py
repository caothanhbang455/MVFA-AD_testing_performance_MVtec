...
def load_mvtec(category, k_shot, experiment_indx):
    ...
    if k_shot == 0:
        ...
        seed_file = os.path.join('dataset/seeds_mvtec', category, 'selected_samples_per_run.txt')
...
class MVTECDataset(Dataset):
    ...
    def __init__(self, load_function, category = 'bottle', k_shot = 5,
                 experiment_indx=0, resize=240):
        ...
        # load datasets
        (self.normal_img_paths, _, _, _), \
        (self.x, self.mask, self.y, _) = load_function(self.category, k_shot, experiment_indx)
    ...
    def get_few_normal(self):
        ...
        x = self.normal_img_paths

        fewshot_img = []
        for idx in range(self.k_shot):
            image = x[idx]
